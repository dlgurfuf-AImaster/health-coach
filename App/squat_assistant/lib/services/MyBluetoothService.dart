...
class MyBluetoothService {
  ...
  // 💡 쪼개져서 들어오는 문자열 조각들을 모아둘 임시 바구니(버퍼)
  String _dataBuffer = "";
            ...
            for (var c in service.characteristics) {
              if (c.properties.notify) {
                ...
                rxCharacteristic!.lastValueStream.listen((value) {
                  // 1. 들어온 바이트 데이터를 문자열로 변환하여 바구니에 계속 이어 붙임
                  _dataBuffer += utf8.decode(value);

                  // 2. 바구니에 줄바꿈(\n)이 포함되어 있다면 하나의 완벽한 패킷이 완성되었다는 뜻!
  ...
  void _parseAndSend(String raw, Function(List<double> w, List<double> t) callback) {
    try {
      ...
      // 혹시 앞에 쓰레기 값이 붙어있을 수 있으므로 '$'의 위치를 찾아서 그 뒤부터 잘라냅니다.
